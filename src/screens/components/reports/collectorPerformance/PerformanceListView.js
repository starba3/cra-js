...
import { getCollectorPerformanceReport } from 'src/data-access/reports';
...
export default function PerformanceListView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await getCollectorPerformanceReport();
        setTableData(result);
      } catch (error) {
  ...
  const TABLE_HEAD = [
    ...
    { id: 'paidCount', label: Translate("paidCount"), align: 'center' },
    { id: 'unpaidCount', label: Translate("unpaidCount"), align: 'center' },
